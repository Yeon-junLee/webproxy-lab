...
void serve_static(int fd, char *filename, int filesize, char *method, char *version);       // 11.6.C (version을 인자로 받아 요청된 HTTP 버전에 맞도록 출력이 되게 변경)
void get_filetype(char *filename, char *filetype);
void serve_dynamic(int fd, char *filename, char *cgiargs, char *method, char *version);     // 11.6.C (version을 인자로 받아 요청된 HTTP 버전에 맞도록 출력이 되게 변경)
void clienterror(int fd, char *cause, char *errnum, char *shortmsg,                         // 11.6.C (version을 인자로 받아 요청된 HTTP 버전에 맞도록 출력이 되게 변경)
